...
This [RStudio](https://global.rstudio.com/categories/rstudio-ide/) notebook studies a data set made available by **Health Canada** on the total number of COVID-19 cases throughout Canada. In this case, it will be used to study the total number of cases in Newfoundland and Labrador. The purpose of this notebook is to evaluate a particular model which can be used to describe and predict the progression of the disease in the province.
...
From a quick visual inspection, the fit of the curve to total cases of COVID-19 in the province is nearly ideal. This is backed up by an $R^2$ value of 0.9998, very close to 1, indicating that the model is an excellent fit for the data. The root mean squared error (RMSE) is 380.26, relatively low when considered against the scale of total case counts; similarly the mean average error (MAE) is also low.

With respect to cross validation, CV $R^2$ of 0.9864, indicating that the model performs well across different folds. The CV RMSE is considerably higher than the overall RMSE however, indicating that the model might suffer from uneven performance across different subsets of the data. Similarly the CV MAE is also higher than the overall MAE, indicating that the model is more accurate on the full data set than on different subsets of it.

Finally, the results of the Breusch-Pagan test confirm that there is no major evidence of heteroscedasticity. A residuals plot should always be examined to confirm or deny this.

The blue region, after the 1600 day mark, indicates where the model has predicted the total number of cases for the next 365 days. This demonstrates the model's ability to provide future forecasting based on little more than a total daily case count.
